...
fn run_with_large_test_stack<F>(future: F) -> Result<()>
where
...
fn open_agent_picker_keeps_missing_threads_for_replay() -> Result<()> {
    run_with_large_test_stack(async {
...
fn make_test_app() -> Pin<Box<dyn Future<Output = App>>> {
    Box::pin(async {
...
fn make_test_app_with_channels() -> Pin<
    Box<
        dyn Future<
            Output = (
                App,
